        ...
        private DateTime? ParseDate(string date, out DateTime newDate)
        {
            ...
            foreach (CultureInfo cultureInfo in CultureInfo.GetCultures(CultureTypes.AllCultures))
            {
                try
                {
                    ...
                    Console.WriteLine(String.Format("Parse date non riuscito, culture : {0}",cultureInfo));
                }
            }

            if (DateTime.TryParse(date, CultureInfo.CurrentUICulture, DateTimeStyles.None, out newDate))
            {
                return newDate;
            }
            //}
